...
var CASPER_ABI = require('./utils/casper_abi.js');
var CASPER_ADDRESS = '0xbd832b0cd3291c39ef67691858f35c71dfb3bf21';
var EPOCH_LENGTH = 50;
...
var UPDATE_CASPER_INTERVAL = 20000;
var MINERS_LIMIT = 5;
...
function Node ()
{
	...
	this.stats = {
		...
		casper:{
			...
			current_epoch_hash:'?',
			last_finalized_epoch_hash:'?',
			last_justified_epoch_hash:'?'
		},
		syncing: false,
		uptime: 0
	};
	...
	this.updateCasperInterval = false;
...
Node.prototype.casper_utils = {
	to_number_callback: function(callback){
							return function(err, data){
								callback(err, data.toNumber())
							}
						},
	to_ether_callback: function(callback){
							return function(err, data){
								callback(err, web3.fromWei(data, "ether").toNumber())
							}
						}
}

Node.prototype.getCasper = function ()
{
	var self = this;

	if(this._web3)
	{
		var timeString = 'Got casper info in' + chalk.reset.red('');
		console.time('==>', timeString);

		var casper = web3.eth.contract(CASPER_ABI).at(CASPER_ADDRESS);
 
		async.parallel({
			current_epoch: function(callback){
				casper.get_current_epoch(self.casper_utils.to_number_callback(callback))
			},
			expected_source_epoch: function(callback){
				casper.get_expected_source_epoch(self.casper_utils.to_number_callback(callback))
			},
			cur_deposits:function(callback){
				casper.get_total_curdyn_deposits(self.casper_utils.to_ether_callback(callback))
			},
			prev_deposits:function(callback){
				casper.get_total_prevdyn_deposits(self.casper_utils.to_ether_callback(callback))
			},
			last_justified_epoch: function(callback){
				casper.get_last_justified_epoch(self.casper_utils.to_number_callback(callback))
			},
			last_finalized_epoch: function(callback){
				casper.get_last_finalized_epoch(self.casper_utils.to_number_callback(callback))
			},
			validator_index: function(callback){
				//try/catch
				casper.get_validator_indexes(self.info.coinbase, self.casper_utils.to_number_callback(callback))
			},
			validator_total: function(callback){
				casper.get_nextValidatorIndex(self.casper_utils.to_number_callback(callback))
			}
		},
		function (err, results)
		{
			if (err) {
				console.error('xx>', 'getCasper error: ', err);
				return false;
			}

			self.stats.casper = results;

			async.parallel([
				function(callback){
					self.getEpochBlocks(results, callback)
				},
				function(callback){
					self.getValidatorInfo(results, callback)
				},
				function(callback){
					self.getCasperInfo(results, callback)
				}
			], function(err, results){
				self.sendCasperUpdate();
			})
		});
	}
}
...
Node.prototype.getCasperInfo = function(casper_results, callback){
	...
	if(this._web3)
	{
		...
		async.parallel({
			cur_dyn_votes: function(callback){ 
				casper.get_votes__cur_dyn_votes(casper_results.current_epoch, casper_results.expected_source_epoch, self.casper_utils.to_number_callback(callback))
 				},
			prev_dyn_votes: function(callback){ 
				casper.get_votes__prev_dyn_votes(casper_results.current_epoch, casper_results.expected_source_epoch, self.casper_utils.to_number_callback(callback))
 				},
			last_nonvoter_rescale: function(callback){
				casper.get_last_nonvoter_rescale(self.casper_utils.to_number_callback(callback))
			},
			last_voter_rescale: function(callback){
				casper.get_last_voter_rescale(self.casper_utils.to_number_callback(callback))
			}
		},
		function(err, casper_results)
		{
			if (err) {
				console.error('xx>', 'getCasperInfo error: ', err);
				return false;
			}

			self.stats.casper.cur_dyn_votes = casper_results.cur_dyn_votes
		    self.stats.casper.prev_dyn_votes = casper_results.prev_dyn_votes
		    self.stats.casper.last_nonvoter_rescale = casper_results.last_nonvoter_rescale
		    self.stats.casper.last_voter_rescale = casper_results.last_voter_rescale
...
Node.prototype.getValidatorInfo = function(casper_results, callback){
	...
	if(this._web3)
	{
		...
		async.parallel({
			start_dynasty: function(callback){ 
				casper.get_validators__start_dynasty(casper_results.validator_index, self.casper_utils.to_number_callback(callback))
			},
			end_dynasty: function(callback){
				casper.get_validators__end_dynasty(casper_results.validator_index, self.casper_utils.to_number_callback(callback))
			},
			address: function(callback){
				casper.get_validators__addr(casper_results.validator_index, callback)
			},
			withdrawal_addr: function(callback){
				casper.get_validators__withdrawal_addr(casper_results.validator_index, callback)
			},
			deposit_unscaled: function(callback){
				casper.get_validators__deposit(casper_results.validator_index, self.casper_utils.to_number_callback(callback))
			},
			deposit_scaled: function(callback){
				casper.get_deposit_scale_factor(casper_results.current_epoch, self.casper_utils.to_number_callback(callback))
			},
		},
		function(err, validator_results)
		{
			if (err) {
				console.error('xx>', 'getValidator error: ', err);
				return false;
			}
			 
			self.stats.casper.start_dynasty = validator_results.start_dynasty
		    self.stats.casper.end_dynasty = validator_results.end_dynasty
		    self.stats.casper.address = validator_results.address
		    self.stats.casper.withdrawal_addr = validator_results.withdrawal_addr
		    self.stats.deposit_unscaled = validator_results.deposit_unscaled 
		    self.stats.casper.deposit_scaled = validator_results.deposit_unscaled * validator_results.deposit_scaled 
...
Node.prototype.getEpochBlocks = function(casper_results, callback){

	var self = this;

	if(this._web3)
	{
		var timeString = 'Got epoch blocks info in' + chalk.reset.red('');
		console.time('==>', timeString);
		async.parallel({
			current_epoch_block: function(callback){
				web3.eth.getBlock(casper_results.current_epoch * EPOCH_LENGTH, callback)
			},
			last_justified_epoch_block: function(callback){
				web3.eth.getBlock(casper_results.last_justified_epoch * EPOCH_LENGTH, callback)
			},
			last_finalized_epoch_block: function(callback){
				web3.eth.getBlock(casper_results.last_finalized_epoch * EPOCH_LENGTH, callback)
			},
		},
		function (err, block_results)
		{
			...
			self.stats.casper.current_epoch_hash = block_results.current_epoch_block.hash;
			self.stats.casper.last_justified_epoch_hash = block_results.last_justified_epoch_block.hash;
			self.stats.casper.last_finalized_epoch_hash = block_results.last_finalized_epoch_block.hash;
			callback(err, null)
		});
	}
}
...
Node.prototype.prepareCasper = function ()
{
	return {
		id: this.id,
...
Node.prototype.sendCasperUpdate = function()
{
	this._lastCasperSentAt = _.now();
	console.stats("wsc", "Sending", chalk.reset.red("casper"), chalk.bold.white("update"));
	var stats = this.prepareCasper();
	console.info(stats);
	this.emit('casper', stats);
}
...
Node.prototype.setWatches = function()
{
	...
	if( !this.updateCasperInterval)
	{
		this.updateCasperInterval = setInterval( function(){
			self.getCasper();
		}, UPDATE_CASPER_INTERVAL);
	}
...
Node.prototype.stop = function()
{
	...
	if(this.updateCasperInterval)
		clearInterval(this.updateCasperInterval);
