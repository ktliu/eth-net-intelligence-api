...
Node.prototype.getCasper = function ()
{
	...
	if(this._web3)
	{
		...
		var to_number_callback = function(callback){
			return function(err, data){
				callback(err, data.toNumber())
			}
		}

		var to_ether_callback = function(callback){
			return function(err, data){
				callback(err, web3.fromWei(data, "ether").toNumber())
			}
		}
		...
		async.parallel({
			current_epoch: function(callback){
				casper.get_current_epoch(to_number_callback(callback))
			},
			expected_source_epoch: function(callback){
				casper.get_expected_source_epoch(to_number_callback(callback))
			},
			cur_deposits:function(callback){
				casper.get_total_curdyn_deposits(to_ether_callback(callback))
			},
			prev_deposits:function(callback){
				casper.get_total_prevdyn_deposits(to_ether_callback(callback))
			},
			last_justified_epoch: function(callback){
				casper.get_last_justified_epoch(to_number_callback(callback))
			},
			last_finalized_epoch: function(callback){
				casper.get_last_finalized_epoch(to_number_callback(callback))
			},
			validator_index: function(callback){
				//try/catch
				casper.get_validator_indexes(self.info.coinbase, to_number_callback(callback))
			}
		},
...
Node.prototype.getCasperInfo = function(casper_results, callback){
	...
	if(this._web3)
	{
		...
		async.parallel({
			/*======= TODO: Decimal10 Error ============*/

			// cur_dyn_votes: function(callback){ 
			// 	casper.get_votes__cur_dyn_votes(casper_results.current_epoch, casper_results.expected_source_epoch, callback)
 			// 	},
			// prev_dyn_votes: function(callback){ 
			// 	casper.get_votes__prev_dyn_votes(casper_results.current_epoch, casper_results.expected_source_epoch, callback)
 			// 	},
			// last_nonvoter_rescale: function(callback){
			// 	casper.get_last_nonvoter_rescale(callback)
			// },
			// last_voter_rescale: function(callback){
			// 	casper.get_last_voter_rescale(callback)
			// }, 
		},
...
Node.prototype.getValidatorInfo = function(casper_results, callback){
	...
	if(this._web3)
	{
		...
		async.parallel({
			...
			// 	casper.get_validators__deposit(casper_results.validator_index, callback)
			// },
			// deposit_scaled: function(callback){
			// 	casper.get_deposit_scale_factor(casper_results.current_epoch)
			// },
		},
